-- Max Pain Analysis View for Utilities Sector
-- Same methodology as airlines analysis
...
WITH
-- Get utility symbols list
utility_symbols AS (
    SELECT DISTINCT symbol
    FROM option_symbol_summary
    WHERE symbol IN ('AEE', 'AEP', 'AES', 'ATO', 'AWK', 'BEPC', 'CEG', 'CNP', 'D', 'DTE',
                     'DUK', 'ED', 'EIX', 'ES', 'ETR', 'EVRG', 'EXC', 'FE', 'GEV', 'NEE',
                     'NI', 'NRG', 'PCG', 'PEG', 'PPL', 'SO', 'SRE', 'UGI', 'VST', 'WEC', 'XEL', 'XLU')
),
-- Calculate symbol-specific Monday baselines for volume (same period as option_symbol_summary)
monday_volume_baseline AS (
    SELECT
        hp.symbol,
        AVG(hp.volume) as baseline_mon_vol
    FROM historical_prices hp
    INNER JOIN utility_symbols us ON hp.symbol = us.symbol
    WHERE strftime('%w', hp.trade_date) = '1'
    AND hp.trade_date >= (SELECT MIN(trade_date) FROM option_symbol_summary WHERE symbol IN (SELECT symbol FROM utility_symbols))
    AND hp.trade_date <= (SELECT MAX(trade_date) FROM option_symbol_summary WHERE symbol IN (SELECT symbol FROM utility_symbols))
    GROUP BY hp.symbol
),
...
friday_volume_baseline AS (
    SELECT
        hp.symbol,
        AVG(hp.volume) as baseline_fri_vol
    FROM historical_prices hp
    INNER JOIN utility_symbols us ON hp.symbol = us.symbol
    WHERE strftime('%w', hp.trade_date) = '5'
    AND hp.trade_date >= (SELECT MIN(trade_date) FROM option_symbol_summary WHERE symbol IN (SELECT symbol FROM utility_symbols))
    AND hp.trade_date <= (SELECT MAX(trade_date) FROM option_symbol_summary WHERE symbol IN (SELECT symbol FROM utility_symbols))
    GROUP BY hp.symbol
),
...
monday_oi_baseline AS (
    SELECT
        oss.symbol,
        AVG(oss.total_open_interest) as baseline_mon_oi
    FROM option_symbol_summary oss
    INNER JOIN utility_symbols us ON oss.symbol = us.symbol
    WHERE strftime('%w', oss.trade_date) = '1'
    GROUP BY oss.symbol
),
...
friday_oi_baseline AS (
    SELECT
        oss.symbol,
        AVG(oss.total_open_interest) as baseline_fri_oi
    FROM option_symbol_summary oss
    INNER JOIN utility_symbols us ON oss.symbol = us.symbol
    WHERE strftime('%w', oss.trade_date) = '5'
    GROUP BY oss.symbol
),
...
weekly_pairs AS (
    ...
    INNER JOIN utility_symbols us ON mon.symbol = us.symbol
