"""
Extended threshold sweep including 2.0 and 2.5, plus methodology transparency.
"""
...
cursor = conn.execute('''
    SELECT symbol, significance_score, premium_value, volume_surprise_factor,
           alert_reason, option_type, alert_timestamp,
...
# But wait - we only have alerts that ALREADY passed the current 6.0 threshold
# (with double smart money). So scores below ~3.5 no-smart are the bottom of
# what the current system even produced. We can't see what a 2.0 threshold
# would catch because those contracts never became alerts.
#
# Let's show this clearly.

print('IMPORTANT CAVEAT:')
print('Our dataset only contains contracts that ALREADY fired as alerts under')
print('the current scoring (double smart money, threshold 6.0). The no-smart')
print('score for these ranges from ~2.5 to ~8.0. We CANNOT see what a 2.0')
print('threshold would additionally catch, because those contracts were never')
print('recorded as alerts.')
print()

# Show the actual score distribution
from collections import Counter
score_buckets = Counter()
for a in alerts:
    bucket = round(a['score_no_smart'] * 2) / 2  # round to nearest 0.5
    score_buckets[bucket] += 1

print('No-smart score distribution of existing alerts:')
print(f'{"Score":>6} {"Count":>7} {"Bar"}')
for score in sorted(score_buckets.keys()):
    count = score_buckets[score]
    bar = '#' * (count // 15)
    print(f'{score:>6.1f} {count:>7}  {bar}')

# Find the minimum
min_score = min(a['score_no_smart'] for a in alerts)
max_score = max(a['score_no_smart'] for a in alerts)
print(f'\nMin no-smart score in dataset: {min_score:.2f}')
print(f'Max no-smart score in dataset: {max_score:.2f}')
print()

# Per-band breakdown including lowest bands
print('=' * 115)
print(f'PER-BAND QUALITY (no smart money) - including lowest available bands')
print(f'Win = option price peaks >= {WIN}% at any point in 7-day window')
print('=' * 115)
print()
...
bands = [
    (7.0, 10.1), (6.5, 7.0), (6.0, 6.5), (5.5, 6.0), (5.0, 5.5),
    (4.5, 5.0), (4.0, 4.5), (3.5, 4.0), (3.0, 3.5), (2.5, 3.0), (2.0, 2.5),
]
...
# Cumulative including low thresholds
print()
print('=' * 115)
print('CUMULATIVE VIEW (extended)')
print('=' * 115)
print()
...
for thresh in [6.0, 5.5, 5.0, 4.5, 4.0, 3.5, 3.0, 2.5, 2.0]:
    ...
    prems = sorted(a['premium_value'] or 0 for a in passing)
    med = prems[len(prems) // 2]

    print(f'{thresh:<10.1f} {len(passing):>6} {per_day:>6.1f} | '
          f'{w3:>6.0f}% {a3:>+7.1f}% {l3:>+7.1f}% | '
          f'{w7:>6.0f}% {a7:>+7.1f}% {l7:>+7.1f}% | '
          f'{ap:>12,.0f} {med:>12,.0f}')


# Show what 2.5-3.0 and 3.0-3.5 alerts look like
print()
print('=' * 90)
print('SAMPLE ALERTS: 2.5-3.5 band (the lowest-scoring alerts in our dataset)')
print('=' * 90)
print()

low = [a for a in alerts if 2.5 <= a['score_no_smart'] < 3.5 and a['max_prof_7d_pct'] is not None]
low.sort(key=lambda x: x['score_no_smart'])
