"""Earnings Calendar Screen - 90 Day View (CSS Grid, scrollable)"""
...
class DayCell90(Static):
    """Individual day cell widget - compact version for 90-day view"""
    ...
    DayCell90 .day-header {
        text-style: bold;
        color: $text;
    }

    DayCell90 .earnings-item {
        margin-left: 1;
    }

    DayCell90 .earnings-item-strong-buy {
        color: $success;
        text-style: bold;
    }

    DayCell90 .earnings-item-buy {
        color: $success;
    }

    DayCell90 .earnings-item-watch {
        color: $warning;
    }

    DayCell90 .count-summary {
        color: $text-muted;
        margin-top: 1;
        text-style: italic;
    }

    DayCell90 .overflow-indicator {
        color: $text-muted;
        text-style: dim;
    }
    """
    ...
    def render(self) -> str:
        """Render day cell content - compact format"""
        ...
        # Show up to 3 earnings (more compact than 20-day view)
        max_display = 3
        ...
        if overflow_count > 0:
            lines.append(f"[dim]+{overflow_count}[/dim]")

        # Count summary
        if len(self.earnings_list) == 0:
            lines.append("[dim]none[/dim]")
        else:
            lines.append(f"[dim]{len(self.earnings_list)}[/dim]")
...
class EarningsCalendar90DayScreen(Screen):
    """Earnings calendar - 90 day view (approximately 13 weeks)"""
    ...
    def _build_calendar(self) -> None:
        """Build calendar grid with day cells - 60 trading days (~90 calendar days)"""
        data = get_data()

        # Generate next 60 trading days (~90 calendar days = ~13 weeks)
        today = datetime.now()
        trading_days = get_next_trading_days(today, num_days=60)
        ...
        first_date = datetime.strptime(trading_days[0], '%Y-%m-%d').strftime('%b %d')
        last_date = datetime.strptime(trading_days[-1], '%Y-%m-%d').strftime('%b %d')

        info_text = (
            f"[bold cyan]Earnings Calendar - 90 Day View[/bold cyan] "
            f"[dim]({first_date} - {last_date}, {total_tradeable} tradeable)[/dim]\n"
        ...
        for date_str in trading_days:
            ...
            cell = DayCell90(date_str, earnings_list)
            grid.mount(cell)
    ...
    def action_navigate_left(self) -> None:
        ...
        if focused and isinstance(focused, DayCell90):
            try:
