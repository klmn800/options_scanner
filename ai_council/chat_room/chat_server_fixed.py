...
class ChatRoom:
    ...
    def get_ai_responses(self, user_message, user_name):
        """Get responses from all AI personalities in the room"""
        logger.debug(f"AI_RESPONSE: Starting get_ai_responses for room {self.room_id}")
        safe_user_message = safe_log_text(user_message, 100)
        ...
        logger.info(f"AI_RESPONSE: Room {self.room_id} has {len(self.ai_personalities)} personalities to query")
        for p in self.ai_personalities:
            logger.debug(f"AI_RESPONSE: - {p.display_name} ({p.code_name})")
        ...
        for i, personality in enumerate(self.ai_personalities):
            try:
                logger.info(f"AI_RESPONSE: [{i+1}/{len(self.ai_personalities)}] Requesting response from {personality.display_name}")
                logger.debug(f"AI_RESPONSE: About to call personality.ask_for_chat() for {personality.code_name}")
                
                # Make the actual API call
                response = personality.ask_for_chat(context)
                
                # Use safe logging to avoid Unicode crashes
                safe_response = safe_log_text(response, 200)
                ...
                safe_preview = safe_log_text(response, 100)
...
def handle_send_message(data):
    ...
    try:
        ...
        safe_message = safe_log_text(message_text, 50)
...
if __name__ == '__main__':
    ...
    print("Available personalities: demo_guru, data_aware_template")
    print("-" * 60)
